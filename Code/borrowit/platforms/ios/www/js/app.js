...
var app = angular.module('starter', ['ionic', 'ngCordova', 'angularMoment', 'satellizer', 'ngCookies'])
  ...
  .controller('UserMessagesCtrl',
    function($scope, $rootScope, $state, $stateParams, $ionicActionSheet,
      ...
      $scope.sendMessage = function(sendMessageForm) {
        var message = {
          text: $scope.input.message.replace(/(<([^>]+)>)/ig,""),
          date: new Date(),
  ...
  .controller("ToDoController", function($scope, $cordovaGeolocation, $ionicPopup, $cordovaVibration, $cordovaCamera, $cordovaDevice, $cordovaFile, $cordovaContacts, $ionicPlatform, $ionicActionSheet, ImageService, ProfileService, ResultService, CommunicationService){
    ...
    $scope.startRequest = function() {
      ...
      var newReqId = CommunicationService.startRequest(newRequest.title, newRequest.description, newRequest.location, newRequest.startDate, newRequest.endDate, newRequest.category);
      newReqId.then(function(result){
        newRequest.requestid = result;
        ProfileService.profile.requests.push({requestid: newRequest.requestid, title: newRequest.title, username: $scope.profile.username});
        window.localStorage.setItem("profile", JSON.stringify(ProfileService.profile));
    ...
    $scope.setRequestFields = function() {
      ...
      if($scope.values.useLocationForRequest) {
        var location = CommunicationService.getLocation();
    ...
    $scope.getAllContacts = function() {
      $cordovaContacts.find().then(function(allContacts) { //omitting parameter to .find() causes all contacts to be returned
        ...
        $scope.deviceContacts = allContacts;
      });
      ...
      for (var i = 0; i < numbers.length; i++)
        for (var j = 0; j < $scope.phoneContacts.length; j++)
          for (var k = 0; k < $scope.phoneContacts[j].phoneNumbers.length; k++)
            if (numbers[i].value == $scope.phoneContacts[j].phoneNumbers[k].value) //work with id
              return true;
      return false;
    }
    ...
    $scope.viewContact = function(contactname)
    {
    ...
    $scope.viewChat = function(req)
    {
      ...
      if(!$scope.checkChatStarted(req.requestid)) //ask backend to get the room
      {
        ...
        $scope.values.currentChat = $scope.chats.length - 1;
      }
      $scope.values.requestOffered = request.offered;
      $scope.values.requestAccepted = request.accepted;
  ...
  .factory('CommunicationService',['$http', '$auth', 'ResultService', 'ProfileService', function($http, $auth, ResultService, ProfileService)
  {
